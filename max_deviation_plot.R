...
max_deviation_plot <- function(data, delta = 0.20){
  ...
  plot <- ggplot(df_plot_sorted, aes(x = Normalized_ID, y = Max_Deviation)) +
    ...
    annotate("text", x = 0.01, y = delta, # X ekseninin en sağına yakın, Y ekseninde delta değeri
             label = bquote(delta == .(paste0(round(delta * 100), "%"))),
             hjust = 0, vjust = -0.5, # Sağa hizala, hafif yukarı kaydır
             color = "black", size = 5) # Çizgi rengiyle aynı renk, uygun boyut
  
  return(plot)
}

plot_aer  <- max_deviation_plot(data = AER, delta = 0.20)
  plot_aer  <- plot_aer + labs(x = NULL) 
plot_bank <- max_deviation_plot(bank, delta = 0.20)
  plot_bank <- plot_bank + labs(x = NULL, y = NULL) 
plot_germ <- max_deviation_plot(german, delta = 0.20)
  plot_germ <- plot_germ + labs(x = NULL, y = NULL) 
plot_give <- max_deviation_plot(give, delta = 0.20)
  plot_give <- plot_give + labs(x = NULL) 
plot_hmeq <- max_deviation_plot(hmeq, delta = 0.20)
  plot_hmeq <- plot_hmeq + labs(x = NULL, y = NULL) 
plot_loan <- max_deviation_plot(loan, delta = 0.20)
  plot_loan <- plot_loan + labs(x = NULL, y = NULL) 
plot_pol3 <- max_deviation_plot(poland3, delta = 0.20)
  plot_pol3  <- plot_pol3
plot_pol5 <- max_deviation_plot(poland5, delta = 0.20)
  plot_pol5 <- plot_pol5 + labs(y = NULL) 
plot_taiw <- max_deviation_plot(taiwan, delta = 0.20)
